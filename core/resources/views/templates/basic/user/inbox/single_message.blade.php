        ...
        @else
            ...
            @if (@$message->media_id || @$message->media_path)
                ...
                @if (@$message->message_type == Status::DOCUMENT_TYPE_MESSAGE)
                    @php
                        // Detectar extensão do arquivo e escolher ícone apropriado
                        $filename = @$message->media_filename ?? '';
                        
                        // Se não tiver media_filename, tenta extrair do media_path (Baileys)
                        if (empty($filename) && !empty(@$message->media_path)) {
                            $filename = @$message->media_path;
                        }
                        
                        $extension = strtolower(pathinfo($filename, PATHINFO_EXTENSION));
                        
                        $documentIcon = 'document_preview.png'; // Ícone padrão
                        
                        if ($extension === 'pdf') {
                            $documentIcon = 'document_pdf_preview.png';
                        } elseif (in_array($extension, ['doc', 'docx'])) {
                            $documentIcon = 'document_doc_preview.png';
                        } elseif (in_array($extension, ['xls', 'xlsx'])) {
                            $documentIcon = 'document_xls_preview.png';
                        }
                    @endphp
                    <div class="text-dark d-flex justify-content-between flex-column">
                        @if (@$message->media_id)
                            <a href="{{ route('user.inbox.media.download', $message->media_id) }}"
                                class="text--primary download-document">
                                <img class="message-image" src="{{ asset('assets/images/' . $documentIcon) }}" alt="image">
                           </a>
                        @else
                            <a href="{{ asset(getFilePath('conversation') . '/' . @$message->media_path) }}"
